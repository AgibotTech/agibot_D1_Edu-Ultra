  ...
  while (true) {
    ...
    if (kbhit()) {
      ...
      case 'w': // 向前 move(vx, vy, yaw_rate) 以0.3m/s的速度向前移动，
                // 侧向速度vy为零， 旋转速度yaw_rate为零, 使能巡逻模式支持超低速控制，默认为false
      ...
      case 'd': // 向右 move(vx, vy, yaw_rate) 以-0.3m/s的速度向右移动，
        ...
        ret = highlevel.move(0.0, -0.3, 0.0);
        break;
      case 'q': // 左转 move(vx, vy, yaw_rate) 以0.3rad/s的速度向左转动，
                // 侧向速度vy为零， 前向速度vx为零
      ...
      case '9': // 切换到双腿站立
        ret = highlevel.twoLegStand(0.0, 0.0);
        break;
      
      case 'z': // 取消双腿站立
        highlevel.cancelTwoLegStand();
        break;
